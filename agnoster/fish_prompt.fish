...
function prompt_dir -d "Display the current directory"
  prompt_segment 4488dd black (prompt_pwd) #blueish
end
...
function prompt_git -d "Display the current git state"
  ...
  if command git rev-parse --is-inside-work-tree >/dev/null 2>&1
    ...
    set git_dirty (command git status --porcelain --ignore-submodules=dirty 2> /dev/null)
    if [ "$git_dirty" != "" ]
      prompt_segment d5dd44 black "$branch" #yellowish
    else
      prompt_segment 88dd44 black "$branch" #greenish
    end
